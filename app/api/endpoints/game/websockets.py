...
async def game_websocket(
    ...
    """Handle active game connections and gameplay interactions."""
    game_state = game_manager.active_games.get(game_id)

    reason = await _is_game_invalid(game_state, current_user.id)
    if reason:
        return await websocket.close(code=4000, reason=reason)
    ...
    try:
        ...
        if opponent and opponent.ws and game_state.status == GameStatus.WAITING:
            await _start_game(game_state)
        await _run_game_loop(game_state, game_view, player, db)
    except WebSocketDisconnect:
        pass
    except RuntimeError:
        pass
...
async def _is_game_invalid(game_state: GameState | None, current_user_id: int):
    # Check if the game exists and is not finished
    if not game_state or game_state.status == GameStatus.FINISHED:
        return "Game not found or already finished"

    # Check if the user is a player in the game
    if current_user_id not in [game_state.player1.user_id, game_state.player2.user_id]:
        return "Not a player in this game"

    # Get the player state and handle reconnection
...
async def _run_game_loop(game_state, game_view, player, db):
    while True:
        try:
            data = await asyncio.wait_for(player.ws.receive_json(), timeout=1.0)
            # No longer accept messages if the game is finished
            if game_state.status == GameStatus.FINISHED:
                break
            await _process_message(game_state, game_view, player, db, data)
        except asyncio.TimeoutError:
            continue
        except Exception as e:
            try:
...
async def _process_message(game_state, game_view, player, db, data):
    if data["type"] == "chat":
        ...
        error = await ability_manager.handle_ability_message(
            game_state, game_manager, player.user_id, data["data"]
        )
        if error:
            await player.ws.send_json({"type": "error", "data": {"message": error}})
    elif data["type"] == "query":
        await player.ws.send_json(
            {"type": "game_state", "data": game_view.model_dump()}
        )
    elif data["type"] == "forfeit":
        await game_manager.forfeit_game(game_state.id, player.user_id)
        await game_manager.handle_game_end(game_state, db)
    elif data["type"] == "submit":
        try:
            await _handle_submit(game_state, player, data, db)
        except SubmittingTooFastError as e:
            await player.send_event(GameEvent(type="error", data={"message": str(e)}))


async def _handle_submit(game_state, player, data, db):
    remaining_cooldown = _remaining_cooldown(game_state, player)
    if remaining_cooldown > 0:
        raise SubmittingTooFastError(remaining_cooldown)
    player.last_submission = time.time()

    result = await _execute_code(game_state, player, data)
    await _process_submission_result(game_state, player, result, db)
...
def _remaining_cooldown(game_state, player):
    ...
    return submission_cooldown - submission_time_diff


async def _execute_code(game_state, player, data):
...
async def _process_submission_result(game_state, player, result, db):
    if result["success"]:
        ...
        await _broadcast_game_state(game_state)
        await _send_next_problem(game_state, player, submission_result)

        # Check if the game has ended and handle it
        if await game_manager.check_game_end(game_state.id):
            await game_manager.handle_game_end(game_state, db)
    else:
        # Submission failed, send the result to the player
        await player.send_event(GameEvent(type="submission_result", data=result))


async def _broadcast_game_state(game_state):
...
async def _send_next_problem(game_state, player, submission_result):
    """Send the next problem to the player if the current one was solved."""
    if (
        submission_result["problem_solved"]
        and player.current_problem_index < len(game_state.problems) - 1
    ):
        next_problem = ProblemManager.prepare_problem_for_client(
            game_state.problems[player.current_problem_index + 1]
        )
        await player.send_event(GameEvent(type="problem", data=next_problem))
