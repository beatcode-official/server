...
class GameManager:
    def __init__(self, challenge_manager: ChallengeManager, websocket_manager: WebSocketManager, docker_execution_service: DockerExecutionService):
        """
    ...
    def create_room(self, player_name: str) -> Dict[str, str]:
        ...
        self.rooms[room_code] = {
            "room_code": room_code,
            "room_status": "Waiting",
            "players": {player_id: [100, 0, 0]},  # [HP, Current Challenge Index, Solved Test Cases]
    ...
    async def start_game(self, room_code: str, player_id: str):
        ...
        room["challenge_ids"] = self.challenge_manager.get_random_indexes(self.challenge_fetch_limit)
    ...
    async def submit_code(self, room_code: str, player_id: str, code: str):
        ...
        challenge = self.challenge_manager.get_challenge(room["challenge_ids"][current_challenge_index])
        ...
        if result["status"] == "success":
            opponent_id = next(pid for pid in room["players"].keys() if pid != player_id)
            new_passed_tests = result["passed_tests"] - room["players"][player_id][2]
            hp_deduction = min(new_passed_tests * self.hp_per_test_case, room["players"][opponent_id][0])
    ...
    async def _send_new_challenge(self, room_code: str, player_id: str = None):
        ...
        for pid in player_ids:
            ...
            challenge = self.challenge_manager.get_challenge(room["challenge_ids"][challenge_index])
            new_challenge = {
                "event": "new_challenge",
                "event_data": {
                    "challenge_info": {
    ...
    async def _send_execution_results(self, room_code: str, player_id: str, result: Dict):
        """
        ...
        execution_results = {
            ...
            "event_data": {
                ...
                "totalTestCases": len(self.challenge_manager.get_challenge(self.rooms[room_code]["challenge_ids"][self.rooms[room_code]["players"][player_id][1]])["test_cases"])
            }
        }
        await self.websocket_manager.send(room_code, player_id, json.dumps(execution_results))
    ...
    async def _end_game(self, room_code: str, winner_id: str):
        ...
        end_game_message = {
            "event": "game_ended",
            "event_data": {
                "winner": room["player_names"][winner_id]
            }
        }
